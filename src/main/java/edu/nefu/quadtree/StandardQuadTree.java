...
public class StandardQuadTree<T> implements Serializable {
        ...
        if (level < minLevel) {
            ...
            if (region == REGION_SELF) {
                ...
                if (zone.contains(x, y)) {
                    ...
                    throw new Exception("[StandardQuadTree][getParentZone] this leaf node doesn't have enough depth. " +
                    ...
                    throw new Exception("[StandardQuadTree][getParentZone] this pixel is out of the quad tree boundary.");
                }
            } else {
    ...
    public Map<Integer,Envelope> getPartitionGrids(){
        HashMap partitionGrides = new HashMap<Integer,Envelope>();
        traverse(new Visitor<T>() {
            private int partitionId = 0;
            ...
            public boolean visit(StandardQuadTree<T> tree) {
                if (tree.isLeaf()) {
                    partitionGrides.put(tree.getZone().partitionId ,tree.getZone().getEnvelope() );
                }
                return true;
            }
        });
        return partitionGrides;
    }
}
