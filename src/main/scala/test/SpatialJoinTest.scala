...
import edu.nefu.DebugQTC.buffer_partition.BufferedPartitioner
...
//      .setMaster("local[*]")
    ...
    val rectangles = "hdfs://master:9000/data/World/rectangles_80M.csv"
  ...
  def spatialJoinFun(sc: SparkContext, leftPath: String, rightPath: String): Unit = {
    ...
    val rightRDD = loader.load(sc, leftPath, FileDataSplitter.WKT)
    ...
    t0 = System.nanoTime()
    val join_result = transleftRDD.spatialJoin(transrightRDD).count
    t1 = System.nanoTime()
